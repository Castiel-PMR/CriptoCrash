...
export function LiquidationCanvas({ 
  ...
  // 🔥 ОПТИМИЗАЦИЯ ПАМЯТИ: Лимиты для предотвращения утечек
  const MAX_PARTICLES = 300; // Максимум частиц одновременно (было бесконечно)
  const MAX_LIQUIDATIONS = 20; // Максимум мешков на экране (было до 100)
  const MAX_PROCESSED_IDS = 200; // Лимит обработанных ID (было 1000)
  const PARTICLE_CLEANUP_INTERVAL = 5000; // Очистка старых частиц каждые 5 сек
  ...
  // Create coin particle for explosion - ОПТИМИЗИРОВАНО
  const createParticle = useCallback((x: number, y: number, isLong: boolean): Particle => {
    return {
      ...
      life: 1,
      decay: Math.random() * 0.02 + 0.015, // 🔥 БЫСТРЕЕ исчезают (было 0.015 + 0.008)
      color: '#ffd700',
  ...
  // Create special particle for clicked bags with 10 different explosion types - ОПТИМИЗИРОВАНО
  const createClickParticle = useCallback((x: number, y: number, isLong: boolean, explosionType: number): Particle => {
    ...
    switch (explosionType) {
      case 0: // Fireworks - яркие разноцветные искры
        ...
        decay = 0.02; // 🔥 УСКОРЕНО (было 0.008)
  ...
  const handleCanvasInteraction = useCallback((clientX: number, clientY: number) => {
    ...
    for (let i = 0; i < state.liquidations.length; i++) {
      ...
      if (!bag.isExploding && 
        ...
        // 🔥 ОПТИМИЗАЦИЯ: Уменьшено количество частиц (было 50, стало 20)
        const particleCount = Math.min(20, Math.floor(bag.width / 3));
  ...
  const updateLiquidationBlock = useCallback((block: LiquidationBlock, deltaTime: number): boolean => {
    ...
    for (let i = 0; i < state.cannonballs.length; i++) {
      ...
      if (ball.targetBagId === block.id) {
        ...
        if (distance < block.width / 2 + 8) { // Ball radius is 8
          ...
          // 🔥 ОПТИМИЗАЦИЯ: Уменьшено количество частиц (было 50, стало 20)
          const particleCount = Math.min(20, Math.floor(block.width / 3));
          for (let j = 0; j < particleCount; j++) {
            state.particles.push(createClickParticle(
    ...
    if (block.y + block.height >= canvas.height - 60) {
      ...
      // 🔥 ОПТИМИЗАЦИЯ: Уменьшено количество частиц (было 15, стало 8)
      const particleCount = Math.min(8, Math.floor(block.width / 6));
  ...
  // 🔥 НОВОЕ: Умное форматирование цены в зависимости от величины (как на Binance)
  ...
  const drawBitcoinChart = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number, opacity?: number) => {
    ...
    // 🔥 ОПТИМИЗАЦИЯ: При большом количестве свечей рисуем каждую вторую
  ...
  const animate = useCallback((currentTime: number) => {
    ...
    if (!isPaused) {
      ...
      // 🔥 ОПТИМИЗАЦИЯ: Лимит частиц на экране (основная утечка памяти!)
  ...
  useEffect(() => {
    ...
    liquidations.forEach(liquidation => {
      // Фильтрация невалидных символов (нелатинские символы, эмодзи и т.д.)
      ...
      if (!processedLiquidations.current.has(uniqueKey)) {
        ...
        if (!exists) {
          try {
            ...
            // 🔥 ОПТИМИЗАЦИЯ: Агрессивная очистка Set (было 1000->500, стало 200->100)
  ...
  // 🔥 ОПТИМИЗАЦИЯ: Периодическая агрессивная очистка памяти
  useEffect(() => {
    const memoryCleanup = setInterval(() => {
      const state = animationStateRef.current;
      
      // Очищаем старые частицы (старше 10 секунд)
      const now = Date.now();
      state.particles = state.particles.filter(p => (now - p.createdAt) < 10000);
      
      // Очищаем processedLiquidations если слишком много
      if (processedLiquidations.current.size > MAX_PROCESSED_IDS) {
        const entries = Array.from(processedLiquidations.current);
        processedLiquidations.current = new Set(entries.slice(-(MAX_PROCESSED_IDS / 2)));
      }
      
      console.log('🔥 Memory cleanup: particles=', state.particles.length, ', liquidations=', state.liquidations.length, ', processed=', processedLiquidations.current.size);
